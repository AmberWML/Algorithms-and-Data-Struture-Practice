...
def optimize_wings(price_list,num_wings):
    #cost[0...total_len]
    #split[0..total_len]
    n=price_list[len(price_list)-1].wings
    if n < num_wings:
        n = num_wings
    ...
    for j in range(1,n+1):
        ...
        for i in range(0,len(price_list)):
            if j-price_list[i].wings>=0 and best > price_list[i].price + cost[j-price_list[i].wings]:
                best = price_list[i].price + cost[j-price_list[i].wings]
                split[j] = i
        cost[j] = best
	#result.append(c[i])
    mini = cost[num_wings]
    a=num_wings
    for k in range (num_wings+1,n+1):
        print(k)
        if cost[k]< mini:
            mini=cost[k]
            a=k
    ...
    while a>0:
        ...
        t=price_list[split[a]].wings
        result.append(price_list[split[a]])
        a=a-t
    return result
