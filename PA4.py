...
def insert_task(task_heap,task):
    ...
    increase_task_priority(task_heap,i,task)
...
def extract_max_priority_task(task_heap):
    ...
    task_heap[0] = task_heap[len(tack_heap)-1]
    max = task_heap.pop()
    Max-Heapify(task_heap,1)
    return max
...
def increase_task_priority(task_heap,i,new_priority):
    if new_priority < task_heap[i]
	return "error “new key smaller than old key” "
    task_heap[i] = new_priority
    while i > 1 and task_heap[parent(i)] < task_heap[i]
	exchange task_heap[i] with task_heap[parent(i)]
	i = parent(i)
