...
def database_insert(database,robot):
    #for name hashtable
    slot_name = hash_function(robot.name)
    database.name_table[slot_name].append(robot)
    #for shape hashtable
    slot_shape = hash_function(robot.shape)
    database.shape_table[slot_shape].append(robot)
    #for color hashtable
    slot_color = hash_function(robot.color)
    database.color_table[slot_color].append(robot)
...
def database_search(database,field,value):
    result = []
    if field == 'name':
        slot = hash_function(value)	
        for i in range(len(database.name_table[slot])):
            x=database.name_table[slot][i]
            if x.name == value:
                result.append(x)
    if field == 'shape':
        slot = hash_function(value)	
        for i in range(len(database.shape_table[slot])):
            x=database.shape_table[slot][i]
            if x.shape == value:
                result.append(x)
       
    if field == 'color':
        slot = hash_function(value)	
        for i in range(len(database.color_table[slot])):
            x=database.color_table[slot][i]
            if x.color == value:
                result.append(x)
    return result
...
def database_delete(database,field,value):
    if field == 'name':
        ...
        for i in range(len(database.name_table[slot])):
            ...
            if x.name == value:
                ...
                shape_slot = hash_function(x.shape)
                database.shape_table[shape_slot].remove(x)
                color_slot = hash_function(x.color)
                database.color_table[color_slot].remove(x)
        for x in l:
            database.name_table[slot].remove(x)
    ...
    if field == 'shape':
        ...
        for i in range(len(database.shape_table[slot])):
            x=database.shape_table[slot][i]
            if x.shape == value:
                #database.shape_table[slot].remove(x)
                l.append(x)
                
                #delete from other tables
                color_slot = hash_function(x.color)
                database.color_table[color_slot].remove(x)
                name_slot = hash_function(x.name)
                database.name_table[name_slot].remove(x)
        for x in l:
            database.shape_table[slot].remove(x)
    ...
    if field == 'color':
        ...
        for i in range(len(database.color_table[slot])):
            ...
            if x.color == value:
                ...
                database.name_table[name_slot].remove(x)
        for x in l:
            database.color_table[slot].remove(x)
