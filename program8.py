...
class something():
    def __init__(self,point,close,far):
        self.point = point
        self.close = close
        self.far = far
...
def nearest_neighbor(kd_tree,point):
    ...
    if kd_tree.root == None:
        return best
    #if k=1 then search back the subtree recursively  
    if k==1:
        k=0
        if cur == kd_tree.root:
            if close == "r" and far =="l":
                cur = kd_tree.root.left
            else:
                cur = kd_tree.root.right
    else:
        cur = kd_tree.root
        best = kd_tree.root
        dist2 = kd_tree.root.dist(point)
    ...
    while cur!= None:
        ...
        if cur.depth%2 == 0:
            ...
            if point.y< cur.y:
                ...
                far="l"
        depth=depth+1
        element=something(prev,close,far)
        path.append(element)
    #check back all the node if needs
    if prev.left == None or prev.right == None:
        a  = 0
        while len(path)!= 0:
            a= a +1
            if a > 1:
            ...
            if depth %2 ==0  and close == "r" and far== "l":
                cur = cur.left
                if cur != None:
                    if abs(cur.x-point.x)<dist1:
                        k = 1
                        nearest_neighbor(kd_tree,point)
            elif depth %2 ==0  and close == "l" and far == "r":
                cur = cur.right
                if cur != None:
                    if abs(cur.x-point.x)< dist1:
                        k = 1
            ...
            elif depth %2 ==1  and close == "l" and far == "r":
                cur = cur.right
                if cur != None:
                    if abs(cur.y-point.y) <dist1:
                        k = 1
                        nearest_neighbor(kd_tree,point)
            
            elif depth %2 ==1  and close== "r" and far == "l":
                cur = cur.left
                if cur != None:
                    if abs(cur.y-point.y)< dist1:
                        k = 1
