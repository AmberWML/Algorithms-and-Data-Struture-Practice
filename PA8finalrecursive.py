...
cur=None
best=None
def NN(best,cur,point):
        prev=cur
        if cur.dist(point)<best.dist(point):
            best = cur
        if cur ==  None:
            return best
        elif depth%2 == 0:
            if point.x<cur.x:
               NN(best,cur.left,point)
               if abs(point.x-cur.left.x)<abs(point.x-best.x):
                   NN(best,cur.left.right,point)
               depth = depth+1
            else:
                NN(best,cur.right,point)
                if abs(point.x-cur.right.x)<abs(point.x-best.x):
                   NN(best,cur.right.left,point)
                depth = depth+1
        else:
            if point.y<cur.y:
                NN(best,cur.left,point)
                if abs(point.y-cur.left.y)<abs(point.y-best.y):
                   NN(best,cur.left.right,point)
                depth = depth+1
            else:
                NN(best,cur.right,point)
                if abs(point.y-cur.right.y)<abs(point.y-best.y):
                    NN(best,cur.right.left,point)
                depth = depth+1
        return best
...
def nearest_neighbor(kd_tree,point):
    ...
    return NN(kd_tree.root,kd_tree.root,point)
