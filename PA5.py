...
def SortStudents(studentList):
    ...
    for j in range(len(studentList)):
        C[studentList[j].year-1] = C[studentList[j].year-1]+1
    for i in range(1,k+1):
        C[i] = C[i] + C[i-1]
    for j in reversed(range(len(studentList))):
        B[C[studentList[j].year-1]-1] = studentList[j]
        C[studentList[j].year-1] = C[studentList[j].year-1]-1
    l = 3
    D = list(B)
    E = [0]*(k+1)
    for j in range(len(B)):
        E[getIndex(B[j].house)] = E[getIndex(B[j].house)]+1
    for i in range(1,l+1):
        E[i] = E[i] + E[i-1]
    for j in reversed(range(len(B))):
        D[E[getIndex(B[j].house)]-1] = B[j]
        E[getIndex(B[j].house)] = E[getIndex(B[j].house)]-1
