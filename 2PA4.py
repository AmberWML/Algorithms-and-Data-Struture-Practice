...
def insert_task(task_heap,task):
    ...
    increase_task_priority(task_heap,i,new_priority)
    #task_heap = task_heap.append(task)
    pass 
...
def  Max_Heapify(task_heap,a):
    L = 2*a+1
    R = 2*a+2
    if L <= len(task_heap)-1 and task_heap[L].priority > task_heap[a].priority:
        largest = L
    else:
        largest = a							
    if R <= len(task_heap)-1 and task_heap[R].priority > task_heap[largest].priority:
        largest = R					
    if largest != a:
        task_heap[a],task_heap[largest] = task_heap[largest],task_heap[a]
...
def increase_task_priority(task_heap,i,new_priority):
    ...
    while i > 0 and task_heap[math.floor((i-1)/2)].priority< task_heap[i].priority:
        task_heap[i],task_heap[math.floor((i-1)/2)] = task_heap[math.floor((i-1)/2)],task_heap[i]
        i = math.floor((i-1)/2)
