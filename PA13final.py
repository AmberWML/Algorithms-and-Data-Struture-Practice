...
def hyperspace_jump_route(star_list,jump_matrix):
    ...
    while  len(Q) != 0:
        ...
        for v in u.adj:
            if v.get_dist() > u.get_dist() + u.jump_times[v.index]:
                v.set_dist(u.get_dist() + u.jump_times[v.index])
                i = Q.index(v)
                heap_decrease_key(Q,i,v.get_dist())
                v.prev = u
            if u.color == "orange":
                revpath.append(u)
                x = revpath[0]
                while x.prev != None:
                    revpath.append(x.prev)
                    x = x.prev
                for i in reversed(range (len(revpath))):
                    path.append(revpath[i])
                    print(revpath[i])
                return path
